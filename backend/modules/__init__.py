...
def load_module_router(module_name: str) -> Optional[APIRouter]:
    ...
    try:
        if module_name == "intake":
            ...
            from modules.whatsapp import router
            logger.info(f"[MODULES] Loaded module: {module_name}")
            return router
        elif module_name == "telegram_alerts":
            from modules.telegram_alerts import router
            logger.info(f"[MODULES] Loaded module: {module_name}")
            return router
        elif module_name == "renting":
            ...
            logger.info(f"[MODULES] Loaded module: {module_name}")
            return router
        elif module_name == "assistencias":
            from modules.assistencias import router
            logger.info(f"[MODULES] Loaded module: {module_name}")
            return router
        else:
