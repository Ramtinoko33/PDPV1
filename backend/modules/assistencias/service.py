...
async def get_employee_for_chat(telegram_user_id: int) -> Optional[dict]:
    ...
    Sprint 2 / S2-A: single source of truth is
    `telegram_internal_authorized_users`. The user_id field links to
    `users.id`. Legacy fallback to `assistencias_bot_users` is removed but
    the collection is preserved on disk (needs_migration path).
    """
    ...
    if not internal_rec:
        return None
    if "assistencias" not in (internal_rec.get("allowed_flows") or []):
        return None

    linked_user_id = internal_rec.get("user_id")
    if linked_user_id:
        user = await db.users.find_one(
            {"id": linked_user_id}, {"_id": 0, "password_hash": 0}
        )
        if user and (
    ...
    # Convivência: registo antigo sem user_id. Devolve minimal dict marcado
    # como needs_migration para que UI/logs possam sinalizar.
    return {
