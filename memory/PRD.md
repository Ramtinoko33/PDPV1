...
## Últimas alterações
- **Feb 2026 (iter 46) — Import UX Hardening (3 features)**:
  - **Cleanup script reforçado**: `backend/scripts/cleanup_corrupt_overdue_pending.py` faz sempre um dump JSON audit em `/tmp/finance_cleanup_backup_<ts>.json` antes de qualquer alteração; `--confirm` só marca imports overdue_balances pending_approval com `totals.documents==0` como `rejected`; não toca em finance_documents/finance_clients.
  - **Modal Pré-Aprovação**: novo endpoint `GET /api/finance/imports/{id}/preview` devolve `{current, new, delta, guard_warnings, is_critical, supported}`. Frontend `FinanceImports.js` abre Dialog shadcn com 3 cartões (Actual/Novo/Δ), guard warnings vermelhos que desabilitam o botão, e input "APROVAR" obrigatório para casos críticos (diff > 30%).
  - **Detecção automática de tipo**: novo endpoint `POST /api/finance/imports/detect-type` com heurística baseada em assinaturas de headers (required*10 + strong). Frontend chama antes do upload e mostra `window.confirm` se o tipo detectado (high/medium confidence) diverge do dropdown seleccionado.
  - Testes novos: `test_iteration_46_import_preview.py` (3/3) + `test_iteration_46b_detect_type.py` (6/6). Regressão iter 43+44: 8/8. Total **17/17 verdes**. Testing agent validou 4/4 cenários UI live no preview.
