...
## Pending
- [ ] P1 Finance: páginas frontend em falta vs spec — Promessas (lista global), Regularizações, Bloqueios (UI de aprovação para FINANCE_REVIEWER), Configurações (thresholds residuais). Endpoints backend já existem para regularizations/promises/block-requests.
- [ ] P1 Finance Fase 2: parser open_documents + comparação diária, pagamentos prováveis/parciais, notas de crédito, promessas cumpridas/falhadas automáticas, relatório semanal.
- [ ] P2 Finance: aviso genérico "Cliente com validação financeira necessária" nos tickets quando cliente BLOQUEADO/CRÍTICO (link finance_clients ↔ customers via linked_customer_id).
- [ ] P3: Excel import
- [ ] P3: Client portal
