...
## Últimas alterações
- **Feb 2026 (iter 45) — Frontend Hardening**: stale-closure em `QuickCommunicationPanel.js` corrigido. `useMemo` de `defaultNumber`/`defaultEmail` agora depende de campos primitivos (`finance_mobile`, `mobile`, `phone`, `finance_email`, `email`) em vez do objecto `client` completo; `useEffect` de interpolação depende de campos primitivos + `templateKey`/`channel`/`templates`; prefill do Telefone/Email só dispara quando o diálogo abre. Edições do utilizador nos inputs deixam de ser apagadas por re-renders do parent. `TasksToday.js`/`TasksEffectiveness.js`/`Regularizations.js` inspeccionados — padrão `useCallback + useEffect` já correcto, sem alterações. Testing agent: 4/4 QuickComm + 3/3 páginas sem loops de fetch + regressão pytest 8/8.
